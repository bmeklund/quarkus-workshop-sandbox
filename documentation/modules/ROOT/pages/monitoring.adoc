...
In the OpenShift console in the topology view for your project, **right-click** on `+Add to Project`, and choose "Container Image"
...
Make sure to change Resource type to deployment - shown below

image::promChangeDeployment.png[Prometheus, 800]

Leave the rest as-is and select *Create*:

You'll see prometheus instance spinning up.
...
Once the build completes, ensure the app completes its redeployment with this command (or watch the Topology View for your project)
...
Follow the same process as before: On the Topology View, select on `+Add to Project`, and choose "Container Image", and fill in the fields:
...
Remember to change the Resource Type:

image::changeDeployment.png[Grafana, 700]

On the Topology View for your project, you'll see Grafana spinning up. Once it completes, select on the arrow to access the Grafana UI:
...
* *URL*: `http://prometheus.%USER_ID%-dev:9090`
...
Back on the Grafana Home page, select *New Dashboard* to create a new _Dashboard_ to review the metrics.
...
* The JVM RSS Value `process_resident_memory_bytes` Set the visualization type to `Gauge` and in the `Standard options` section set Unit field to `Data rate` > `bytes/sec(IEC)`.
Set the title to `Memory` on the _Panel Title_

image::grafanaAddBytesIEC.png[jvm,500]

image::grafanaGauge.png[jvm,500]
